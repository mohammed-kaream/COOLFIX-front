    ...
    $('.counter_feature').on('inview', function (event, visible) {
        if (visible) {
            $(this).find('.counter-num').each(function () {
                var $this = $(this);
                // نجلب الرقم النهائي المستهدف والمخزن في السمة أو النص
                var targetValue = $this.data('target') || $this.text();

                // نخزن الرقم النهائي في data لكي لا نفجده عند إعادة التصفير
                if (!$this.data('target')) {
            ...
            // عندما يخرج القسم عن الشاشة، نعيد تصفير الأرقام لتبدأ من جديد المرة القادمة
            $(this).find('.counter-num').each(function () {
                var $this = $(this);
                if ($this.data('target')) {
                    $this.text(0);
